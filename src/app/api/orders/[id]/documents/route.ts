...
export async function POST(
  ...
  const { name, file_type, uploaded_by, direction, file_url } = body;
  if (!name) return NextResponse.json({ error: "请提供文档名称" }, { status: 400 });

  const result = db.prepare(
    "INSERT INTO documents (order_id, name, file_type, status, direction, uploaded_by, file_url) VALUES (?, ?, ?, '待审核', ?, ?, ?)"
