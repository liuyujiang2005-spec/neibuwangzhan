          ...
          {sidebarTab === "finances" && (
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
              {finances.length === 0 ? <p className="py-4 text-center text-xs text-[var(--muted-foreground)]">暂无费用记录</p> : (
                <ul className="flex flex-col gap-2">
                  {finances.map((f) => (
                    <li key={f.id} className="flex items-center gap-2 rounded-md p-2 transition-colors hover:bg-[var(--secondary)]">
                      ...
                      {isEditing ? (
                        <div className="space-y-2">
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <label className="text-[0.65rem] text-[var(--muted-foreground)]">证书编号</label>
                              <input value={fields.certificate_number || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, certificate_number: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]" />
                            </div>
                            <div>
                              <label className="text-[0.65rem] text-[var(--muted-foreground)]">产品名称</label>
                              <input value={fields.product_name || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, product_name: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]" />
                            </div>
                            <div>
                              <label className="text-[0.65rem] text-[var(--muted-foreground)]">签发日期</label>
                              <input type="date" value={fields.issue_date || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, issue_date: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]" />
                            </div>
                            <div>
                              <label className="text-[0.65rem] text-[var(--muted-foreground)]">到期日期</label>
                              <input type="date" value={fields.expiry_date || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, expiry_date: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]" />
                            </div>
                            <div>
                              <label className="text-[0.65rem] text-[var(--muted-foreground)]">NSW注册号</label>
                              <input value={fields.nsw_registration || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, nsw_registration: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]" />
                            </div>
                            <div>
                              <label className="text-[0.65rem] text-[var(--muted-foreground)]">NSW下载状态</label>
                              <select value={fields.nsw_download_status || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, nsw_download_status: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]">
                                <option value="">—</option>
                          ...
                          <div>
                            <label className="text-[0.65rem] text-[var(--muted-foreground)]">备注</label>
                            <input value={fields.notes || ""} onChange={(e) => setEditCertFields((p) => ({ ...p, notes: e.target.value }))} className="w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-2 py-1 text-xs outline-none focus:border-[var(--ring)]" />
                          </div>
                          <div className="flex gap-1.5">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1.5">
                              <span className={cn("inline-flex rounded-full px-2 py-0.5 text-[0.65rem] font-medium",
                                dynStatus === "expiring" ? "bg-[color-mix(in_oklch,var(--warning),var(--background)_85%)] text-[var(--warning)]" :
                                dynStatus === "expired" ? "bg-[color-mix(in_oklch,var(--destructive),var(--background)_92%)] text-[var(--destructive)]" :
                                "bg-[color-mix(in_oklch,var(--success),var(--background)_85%)] text-[var(--success)]"
                              )}>{dynStatus === "expiring" ? "即将到期" : dynStatus === "expired" ? "已过期" : "有效"}</span>
